...
def apply_gaussian_blur(image: torch.Tensor, kernel_size: int, sigma: float = 1.0) -> torch.Tensor:
    """
    对输入的RGB图像应用高斯模糊。
    
    参数:
    - image (torch.Tensor): 输入图像，形状应为 [3, H, W]。
    - kernel_size (int): 高斯核的大小。必须是奇数。
    - sigma (float): 高斯核的标准差，默认值为1.0。
    
    返回:
    - torch.Tensor: 经过高斯模糊处理后的图像，保持与输入相同的尺寸 [3, H, W]。
    """
    ...
    kernel = gaussian_kernel_2d(kernel_size, sigma).repeat(channels, 1, 1, 1).cuda()
    padding = kernel_size // 2

    blurred_image = F.conv2d(image, kernel, padding=padding, stride=1, groups=channels)

    return blurred_image.squeeze(0) if blurred_image.shape[0] == 1 else blurred_image

def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, pyqt_window):
    ...
    while 1:
        ...
        # Process GUI events after network GUI operations
        app.processEvents()
        ...
        if length_to_process > 0 and iteration % 20 == 0:
            ...
            for i in range(length_to_process):
                ...
                pyqt_window.view1.pointcloud_widget.update_pointcloud_color(xyzs, colors)

                xyzs_list.append(xyzs)
                colors_list.append(colors)
                image, sec1, nanosec1 = receiver.img_queue.get()

                qvec, tvec, sec2, nanosec2 = receiver.pose_queue.get()

                if sec1 != sec2 or nanosec1 != nanosec2:
                    print(f"Pose timestamp mismatch, {sec1}{nanosec1} != {sec2}{nanosec2}")
                ...
                if angle>10 or movement>0.2:
                    ...
                    if scene.online_cameras!= []:
                        new_camera.exposure_param.data = scene.online_cameras[-1].exposure_param.data.clone()
            ...
            if gaussians._xyz.shape[0] > 0 and xyzs_list.shape[0] > 0:
                mask = add_points_with_voxel_limit(gaussians._xyz.data, xyzs_list, 0.1, 50)
            ...
            for i, camera in enumerate(camera_add_list):
                gaussians.exposure_optimizer.add_param_group({"params": camera.exposure_param, "lr": 0.0001, "name": str(camera.id)})
                gaussians.pose_optimizer.add_param_group({"params": [camera.cam_rot_delta], "lr": 0.001, "name": "rot"+str(camera.id)})
                gaussians.pose_optimizer.add_param_group({"params": [camera.cam_trans_delta], "lr": 0.00001, "name": "trans"+str(camera.id)})
            
            # Process GUI events after processing point cloud data
            app.processEvents()

        # 在没有收到任何训练图像的时候，死循环
        if len(scene.online_cameras)<100:# or gaussians._xyz.shape[0]==0:
            # Process GUI events to keep the interface responsive
            app.processEvents()
            continue
        ...
        free_vis_init = False
        if True:
            # if not free_vis_init:
            #     pyqt_window.view2.render_cam.world_view_transform = scene.online_cameras[0].world_view_transform
            #     pyqt_window.view2.render_cam.image_width = scene.online_cameras[0].image_width
            #     pyqt_window.view2.render_cam.image_height = scene.online_cameras[0].image_height
            #     pyqt_window.view2.render_cam.FoVx = scene.online_cameras[0].FoVx
            #     pyqt_window.view2.render_cam.Fovy = scene.online_cameras[0].FoVy
            #     pyqt_window.view2.render_cam.znear = scene.online_cameras[0].znear
            #     pyqt_window.view2.render_cam.zfar = scene.online_cameras[0].zfar
            #     free_vis_init = True
            
            with torch.no_grad():
                vis_pkg = render(pyqt_window.view2.render_cam, gaussians, pipe, background, scaling_modifier=1, use_trained_exp=dataset.train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE)
                vis_image = vis_pkg["render"] + gaussians.bg_color[:, None, None] * vis_pkg["T_left"]
                pyqt_window.view2.update_image((vis_image.detach().clamp(0, 1).permute(1, 2, 0).cpu().numpy()*255).astype(np.uint8))
            
            # Process GUI events after rendering
            app.processEvents()
        ...
        image = image + gaussians.bg_color[:, None, None] * render_pkg["T_left"]
        ...
        if iteration % 1 == 0:
            pyqt_window.view3.update_image((image.detach().clamp(0, 1).permute(1, 2, 0).cpu().numpy()*255).astype(np.uint8))
            pyqt_window.view4.update_image((gt_image.detach().permute(1, 2, 0).cpu().numpy()*255).astype(np.uint8))
            
            # Process GUI events after updating images
            app.processEvents()
        import torchvision
        ...
        kernel_size = 3
        blur_loss = l1_loss(apply_gaussian_blur(image, kernel_size=kernel_size), apply_gaussian_blur(gt_image, kernel_size=kernel_size))
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim_value) + blur_loss # + opacity_reg + scaling_reg
        ...
        with torch.no_grad():
            ...
            if iteration < opt.iterations:
                ...
                gaussians.bg_color_optimizer.step()
                gaussians.bg_color_optimizer.zero_grad(set_to_none = True)
            ...
            if (iteration in checkpoint_iterations):
                ...
                # Process GUI events after saving checkpoint
                app.processEvents()
...
if __name__ == "__main__":
    ...
    training_thread = threading.Thread(target=training, args=(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, window))
